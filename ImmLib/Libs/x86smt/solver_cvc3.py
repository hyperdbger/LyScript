...
class Solver(object):
    ...
    def __init__(self, manualInitialization=False):
        ...
        try:
            CDLL(os.path.join("Libs","x86smt","libgmp-10.dll"))
            self.CVC = CDLL(os.path.join("Libs", "x86smt", 
                                            "libcvc3.2.1.1.dll"))
        except WindowsError:
            try:
                self.CVC = CDLL("libcvc3.2.1.1.dll")
            except WindowsError:
                self.CVC = None
    ...
    def loadExpr(self, dump, recursive=False, varsdict=None):
        ...
        if not recursive:
            self.state={ "vars":{}, "types":{}, "boundVars":{}, "skolemExists":{}, "skolemVars":{} }
            if varsdict:
        ...
        if kind == self._BITVECTOR:
            bvsize = dump[1][0][1]
            if hasattr(self, "bv%dbits"%bvsize):
                return getattr(self, "bv%dbits"%bvsize)
            
            if not self.state["vars"].has_key(self._BITVECTOR+bvsize):
                self.state["vars"][self._BITVECTOR+bvsize]=self.bvType(bvsize)
            return self.state["vars"][self._BITVECTOR+bvsize]
...
def mymain():
    ...
    v1 = sol.constExpr(0xcafecafe)
    v2 = sol.constExpr(0xcafecaff)
    
    eq = sol.eqExpr(v1,v2)
    print sol.assertFormula(eq)
    print sol.queryFormula(eq)
    #print sol.exprString(eq)

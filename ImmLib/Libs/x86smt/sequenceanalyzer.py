...
class StateMachine:
    ...
    def calcComplexity(self, regProps, flagProps):
        ...
        self.solver.push()
        self.solver.assertFormula(self.solver.eqExpr(self.solver.lookupVar("ESP")[0], self.solver.constExpr(0)))
    ...
    def push(self):
        ...
        tosave = ( deepcopy(self.regs), deepcopy(self.flags), deepcopy(self.exceptions), deepcopy(self.memory.sources),\
                   deepcopy(self.memory.undefvalues), self.memory.copy(), self.EIP, self.RETNoffset)
    ...
    def pop(self):
        ...
        #now the VM
        ret = self.stack.pop()
        ( self.regs, self.flags, self.exceptions, self.memory.sources, self.memory.undefvalues, mem, self.EIP, self.RETNoffset) = ret
    ...
    def printRegisters(self, imm):
        ...
        imm.log("EIP:%s"%self.solver.exprString(self.EIP))
        imm.log("-----============= Flags ===============------")
        for k,v in self.flags.iteritems():
            imm.log("%s:%s"%(k,self.solver.exprString(v)))
        imm.log("-----========== Exceptions =============------")
    ...
    def printState(self, imm):
        ...
        imm.log("FLAGS=%s"%mybin(props[1]))
        imm.log("Complexity=%d"%compexity)


class SequenceAnalyzer:
    def __init__(self, imm, r_model=None, f_model=None, m_model=None,
                 follow_jmps=True, analysis_mods=[], simplifyState=True):
        """
        @type r_model: Dict
        @param r_model: A dictionary mapping registers to concrete 
            values. Any registers not described in this dict will
        ...
        self.useCurrentMemory = False
        self.accessed_mem_addrs = set()
        if m_model:
            self.useCurrentMemory = True
        
        self.state = StateMachine(r_model, f_model)

        # If this is True the analyzeJcc function will simply determine
        # which of the jcc outcomes are possible and self.jcc_taken and
        # self.jcc_not_taken are set accordingly. Without this the 
        # solver attempts to follow the jcc and continues its analysis
        # which we may not want if an external script is driving the 
        # analysis.
        self.follow_jmps = follow_jmps
        if not self.follow_jmps:
            self.analyzeJcc = self.analyzeJccNoFollow
    ...
    def solveEIP(self, initialaddress=None):
        ...
        if address == None:
            if self._debug: self.imm.log("Don't know how to solve EIP: %s"%self.state.solver.exprString(simplified))
            self.errors.append("Don't know how to solve EIP: %s"%self.state.solver.exprString(simplified))
    ...
    def analyzeNext(self):
        ...
        if not self.imm.validateAddress(address, "X"):
            if self._debug: self.imm.log("EIP is at a non-executable mempage: 0x%x"%address, address)
            self.errors.append("EIP is at a non-executable mempage: 0x%x"%address)
            if not self.continue_on_error:
                return False
        
        op = operations.operation(self.imm.disasm(address, DISASM_FILE)) #abstraction - skylar
        if not op.validateInstruction():
            if self._debug: self.imm.log("Instruction invalid at 0x%x:'%s'"%(op.getAddress(), op.getDisasm()), op.getAddress())
            self.errors.append("Instruction invalid at 0x%x:'%s'"%(op.getAddress(), op.getDisasm()))
        ...
        if address in self.visitedAddresses:
            if self._debug: self.imm.log("We're analyzing an already visited address, LOOP detected at 0x%x:'%s'"%(op.getAddress(), op.getDisasm()), op.getAddress())
            self.errors.append("We're analyzing an already visited address, LOOP detected at 0x%x:'%s'"%(op.getAddress(), op.getDisasm()))
            if self.continue_on_error:
                return True
            else:
                return False
        ...
        self.state.EIP = self.state.solver.addExpr(self.state.EIP, self.state.solver.constExpr(op.getSize()))
        ...
        if hasattr(self, "analyze%s"%cmd):
            try:
                ...
                #a Jcc was processed, terminate here
                raise StopIteration
            except Exception, reason:
                if self._debug:
                    self.imm.log("Exception while analyzing '%s' at 0x%x: %s" % \
                                 (op.getDisasm(), op.getAddress(), reason), op.getAddress())
                ...
                self.errors.append("Exception while analyzing '%s' at 0x%x: %s"%(op.getDisasm(), op.getAddress(), reason))
                    
                if not self.continue_on_error:
                    return False
        else:
            if not self.analyzeAllUnsupported(op, cmd):
                if self._debug: self.imm.log("Unsupported OPCODE:%s '%s'"%(cmd,op.getDisasm()), op.getAddress())
                self.errors.append("Unsupported OPCODE:%s at 0x%x '%s'"%(cmd,op.getAddress(),op.getDisasm()))
                if self.continue_on_error:
                    return True
                else:
                    return False
        ...
        if ex:
            if self._debug: self.imm.log("Processor Exception arised!! (%s) on %s"%(ex,op.getDisasm()), op.getAddress())
            self.errors.append("Processor Exception arised!! (%s) at 0x%x: '%s'"%(ex,op.getAddress(),op.getDisasm()))
            if self.continue_on_error:
                return True
            else:
                return False
    ...
    def checkExceptions(self):
        ...
        for name,expr in self.state.exceptions.iteritems():
            if self.state.solver.exprString(expr) == "TRUE":
                return name
        return None
    ...
    def analyzeLEA(self, op):
        dst = self.buildState(op.operand[0])
        
        #IDBUG: for some reason this doesn't get set originally
        if op.op2Size() == 2:
            tmpoperand = (DEC_WORD,op.op2Size(),op.op2Register(),op.op2Constant())
        else:
            tmpoperand = (DEC_DWORD,op.op2Size(),op.op2Register(),op.op2Constant())
        src = self.buildState(tmpoperand)
    ...
    def analyzeMOV(self, op):
        dst = self.buildState(op.operand[0])
        src = self.buildState(op.operand[1])
    ...
    def analyzeSHRD(self, op):
        op1 = self.buildState(op.operand[0]);
        op2 = self.buildState(op.operand[1]);
        op3 = self.buildState(op.operand[2]);
    ...
    def analyzeRCL(self, op):
        ...
        """
        dst = self.buildState(op.operand[0]); dstvalue = self.getValueFromState(dst)
        src = self.buildState(op.operand[1]); srcvalue = self.getValueFromState(src)
        srcvalue = self.fixOP2(srcvalue, dst[1])
    ...
    def analyzePUSH(self, op):
        src = self.buildState(op.operand[0])
        size = src[1]
    ...
    def analyzeXCHG(self, op):
        ...
        #get both variables
        var1 = self.buildState(op.operand[0])
        value1 = self.getValueFromState(var1)
        
        var2 = self.buildState(op.operand[1])
    ...
    def analyzeCMP(self, op):
        ...
        dst = self.buildState(op.operand[0]); dstval = self.getValueFromState(dst)
        src = self.buildState(op.operand[1]); srcval = self.getValueFromState(src); srcval = self.fixOP2(srcval, dst[1])
    ...
    def analyzeJccNoFollow(self, condition, finaladdress):
        """
    ...
    def analyzeJcc(self, condition, finaladdress):
        ...
        if not ret:
            ...
            if not self.analyze():
                ...
                self.pop()
                self.errors += errors
                raise Exception, "An error was found while analysing a conditional branch"
            self.push()
            trueState=self.pop()
    ...
    def updateFlags(self, cmd, res, op1=None, op2=None, op3=None):
        ...
        if op2: op2size = self.state.solver.getBitSizeFromExpr(op2)
        if op3: op3size = self.state.solver.getBitSizeFromExpr(op3)
    ...
    def getValueFromState(self, state):
        ...
        if state[0][0] == 'mem':
            ...
            raise Exception, "we are in the oven... UNKNOWN state type"
    ...
    def setValueFromState(self, state, value):
        if state[0][0] == 'mem':
            ...
            raise Exception, "Destination is not memory nor a register, what else can be?" #XXX: we don't support FPU/MMX/SSE/etc yet
    ...
    def fixOP2(self, op2value, op1size):
        ...
        if op2value and self.state.solver.getBitSizeFromExpr(op2value) > op1size:
    ...
    def getAddressFromState(self, state):
        """
        Given a state tuple that describes a memory operand this 
        function computes the effective address.

        @type state: Tuple
        @param state: A tuple describing a memory operand as returned
            by buildState
        """
        ...
        if state[1][0]:
            index_1 = self.state.solver.UConstFromExpr(
                self.state.solver.simplify(state[1][2]))
            # If the solver cannot simplify the expression to a constant
            # then the address being read from/written to is partially
            # or fully undefined. 
            if index_1 is None:
                msg = "First index is undefined %s" % str(state)
                self.imm.log("%s" % msg)
                raise UndefinedMemoryException(msg)
            
            scale_1 = state[1][1]
            addr += index_1 * scale_1

        # add index2 * scale2 (if available)
        if state[1][3]:
            index_2 = self.state.solver.UConstFromExpr(
                self.state.solver.simplify(state[1][5]))
            if index_2 is None:
                msg = "Second index is undefined %s" % str(state)
                self.imm.log("%s" % msg)
                raise UndefinedMemoryException(msg)
            
            scale_2 = state[1][4]
            addr += index_2 * scale_2
    ...
    def setMemoryState(self, stateandsize, value):
        ...
        size = stateandsize[1]
        if state[0] != 'mem':
            raise Exception, "trying to set a Memory space using a non-memory reference"
        ...
        return self.setMemoryStateFromSolverState(tmpstate, value, size, m_addr)
        
    def setMemoryStateFromSolverState(self, state, value, size, m_addr=None):
        tmpstate=state
        ...
        for pos in range(0,size,8):
            if m_addr is not None:
                addr = m_addr + (pos / 8)                
                self.accessed_mem_addrs.add(addr)
    ...
    def getMemoryState(self, stateandsize):
        ...
        size = stateandsize[1]
        if state[0] != 'mem':
            raise Exception, "trying to get a Memory State from a non-memory reference"
        ...
        tmpstate = self.transformMemoryStateToSolverState(state)
        return self.getMemoryStateFromSolverState(tmpstate, size, m_addr)
    
    def getMemoryStateFromSolverState(self, state, size, m_addr=None):
        """
        This function retrieves the value associated with the memory
        ...
        If m_addr is not None it should be the concrete memory address
        that state represents. In this case on the first access to that
        address we read the actual value and associate it with state
        in our memory map. If m_addr is None a default variable
        expression would instead be created. When doing symbolic
        execution this would indicate the memory location is entirely
        under user control.
        ...
        @type m_addr: Int
        @param m_addr: (Optional, Default=None) The concrete memory
            address denoted by state
        ...
        for pos in range(0,size,8):
            if m_addr is not None:
                ...
                if addr not in self.accessed_mem_addrs:
                    self.accessed_mem_addrs.add(addr)
                    val = self.imm.readMemory(addr, 1)

                    if len(val) == 0:
                        raise MemoryOutOfBoundsException(
                            "Error trying to read address %s" % \
                            hex(addr))
                    
                    val = ord(val)
                    val_expr = self.state.solver.constExpr(val, 8)
    ...
    def buildState(self, operand):
        """
        It builds a state representation of an operand.
        
        It takes one of the operand's tuples and returns a 2-tuple:
        ( state, size )
        ...
        if operand[0] == DEC_UNKNOWN:
            raise Exception, "DEC_UNKNOWN type"

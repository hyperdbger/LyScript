...
class operation(opCode):
		...
		for x in range(0,3):
			if self.operand[x][0] & DECR_ISREG == 0 and self.operand[x][0] & DEC_CONST == 0 and \
			   self.operand[x][2] == (0,0,0,0,0,0,0,0) and self.operand[x][0] != 0:
				if x == 0:
					if   cmd in ["BT","CMP","TEST","CMPS","LODS","MUL","IMUL","DIV","IDIV","JMP","CALL"]:
						if not self.imm.validateAddress(self.operand[x][3], "R"):
							return True
					elif cmd in ["CMPXCHG","CMPXCHG8B","BTC","BTR","BTS","XADD"]:
						if self.imm.validateAddress(self.operand[x][3], "RW"):
							return True
					elif not self.imm.validateAddress(self.operand[x][3], "W"):
						return True
				else:
					if cmd == "XADD":
						if not self.imm.validateAddress(self.operand[x][3], "RW"):
							return True
					elif not self.imm.validateAddress(self.operand[x][3], "R"):
						return True
		return False
	...
	def validateInstruction(self):
		...
		#we dont support the REP prefix
		if self.nprefix > 0 and "REP" in completeText:
			return False

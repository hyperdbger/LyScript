...
class DeplibFinder:
    ...
    def setMemFlags(self, addr, size, flags):
        ...
        for x in xrange(0, size):
            if self.memoryFlags.has_key(addr+x) and "F" in self.memoryFlags[addr+x]:
                return False
    ...
    def checkMemAccess(self, addr, perm, size=1):
        ...
        for x in xrange(0, size):
            if not self.memoryFlags.has_key(addr+x):
                return False
            p=self.memoryFlags[addr+x]
            if "N" in p:
                return False
            if perm not in p:
                return False
    ...
    def getEIPptr(self):
        tmp=self.state.memory.sources[str(self.state.EIP[0:8]).replace("VAL","MEM")]
